...
test('契约层判定在搬迁中保持不变（回归护栏）', async () => {
  ...
  assert.equal(c.EXCEL_METHODS.length, 31, '宿主方法路由表不得在搬迁中增减（有意扩展需在此写明理由）');
  ...
  assert.deepEqual(c.HOST_IMPLEMENTATION_GAPS, { wps: ['update_chart'], microsoft: ['get_style_token'] });
});
