...
(function () {
  ...
  window.OnActionBridgeStatus = function () {
    try {
      ...
      if (!isConnected) {
        msg += "\n排查建议:\n1. 确认 WPS Bridge 桌面客户端已启动后台服务；\n2. 若长期无法连通，可在客户端点击【安装 / 升级加载项】。";
      } else {
        msg += "\n提示: 在 Cursor / Claude / WorkBuddy 等 Agent 中直接对话即可操作当前文件。";
      }
      showNativeAlert(msg);
    } catch (e) {
  ...
  setInterval(function () {
    ...
    try {
      const host = detectHostComponent();
      sendPacket({ type: "register", client: host === "word" ? "wps-word-addon" : host === "ppt" ? "wps-ppt-addon" : "wps-et-addon", version: "2.0.0", summary: getWorkspaceSummary(getApp()) });
