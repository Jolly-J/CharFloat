...
其他目录：[docs](docs/) 放架构和验证说明；[prompts](prompts/) 放运行时提示词；[build](build/) 和 resources 的图片为打包素材。它们沿用根规范。当前架构参考 [architecture.md](docs/architecture.md)，主入口配置为 [package.json](package.json) 和 [tsup.config.ts](tsup.config.ts)。
