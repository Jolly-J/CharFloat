...
console.log('── 单元格 ──');
{
  ...
  const v = await host(`return String(app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('A1').Value2);`);
  ...
  const f = await host(`return String(app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('C1').Formula);`);
  const fv = await host(`return String(app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('C1').Value2);`);
...
console.log('── 格式 ──');
{
  ...
  const r = await host(`const c=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('A1'); const o=[]; o.push('填充='+c.Interior.Color); o.push('字色='+c.Font.Color); o.push('粗='+c.Font.Bold); o.push('号='+c.Font.Size); o.push('对齐='+c.HorizontalAlignment); o.push('格式='+c.NumberFormat); return o.join('|');`);
  const s = String(r ?? '');
...
console.log('── 条件格式 ──');
{
  ...
  const r = await host(`const fc=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('B1:B3').FormatConditions; const f=fc.Item(1); return fc.Count+'|'+f.Type+'|'+f.Operator+'|'+f.Formula1+'|'+f.Interior.Color;`);
  const s = String(r ?? '');
...
console.log('── 数据有效性 ──');
{
  ...
  const r = await host(`const v=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Range('D1').Validation; return v.Type+'|'+v.Formula1;`);
...
console.log('── 打印设置 ──');
{
  ...
  const r = await host(`const p=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').PageSetup; return p.PrintArea+'|'+p.Orientation+'|'+p.PaperSize+'|'+p.CenterHorizontally+'|'+p.PrintGridlines;`);
  const s = String(r ?? '');
...
console.log('── 图表（重点：参数是否真的落到宿主）──');
{
  ...
  const probe = await host(`const ws=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}'); let c=null,sh=null; for(let i=1;i<=ws.Shapes.Count;i++){const x=ws.Shapes.Item(i); try{ if(x.HasChart){c=x.Chart;sh=x;break;} }catch(e){}} if(!c) return 'NO_CHART';
    const o=[];
  ...
  const p2 = await host(`const ws=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}'); let c=null,sh=null; for(let i=1;i<=ws.Shapes.Count;i++){const x=ws.Shapes.Item(i); try{ if(x.HasChart){c=x.Chart;sh=x;break;} }catch(e){}} if(!c) return 'NO_CHART';
...
console.log('── 矢量形状 ──');
{
  ...
  const r = await host(`const s=app.Workbooks.Item('${WB}').Worksheets.Item('${SH}').Shapes.Item('ftShape'); const tf=s.TextFrame; const o=[];
