...
| **CAP-12** | **Excel 线程化批注完整语义**：回复、@提及、标记已解决 | Office.js `Comment` | 文档可证 | Office.js 侧 | AI 只能"贴一条"，不能参与讨论 | ✅ **已完成（Word 侧）／❌ Excel 侧宿主做不到**。**Word 侧真机 4/4 通过**：新建批注（正文与作者真存，作者读回=当前用户名）、**标记已解决 `Done`**、**重新打开**、列出批注（含 done 状态）；唯一做不到的是**回复**——`Replies.Add(Range,文本)` 返回对象但 Count 恒为 0，工具**如实报失败**并提示替代做法。**Excel 侧**的 `CommentThreaded` 是空壳（正文不存、回复文本恒为 default、Resolved 写不进），工具已改为如实报失败并指引改用传统批注。⚠️ **更正记录**：此前曾把整个 CAP-12 判为"宿主做不到"，那是**越界结论**——当时只测了 Excel 侧，而 Word 的 `doc.Comments` 是完全不同的另一套 API，由使用者指出后重测推翻 |
