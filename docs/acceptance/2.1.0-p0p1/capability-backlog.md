...
| **CAP-12** | **Excel 线程化批注完整语义**：回复、@提及、标记已解决 | Office.js `Comment` | 文档可证 | Office.js 侧 | AI 只能"贴一条"，不能参与讨论 | ⬜ **待重测（我此前的"宿主做不到"结论下窄了）**。此前只测了 **WPS 表格（Excel）** 的线程批注，探明那套 API 是空壳；但**使用者指出 WPS 文字（Word）侧批注/审阅是可用功能**——Word 的评论体系（`doc.Comments`）与 Excel 的 `CommentThreaded` **是两套完全不同的 API**，不能拿 Excel 的结果给整个 CAP-12 下结论。**待测**：Word 侧新建批注、回复、作者、标记已解决、以及"审阅模式"下的可见性。 |
