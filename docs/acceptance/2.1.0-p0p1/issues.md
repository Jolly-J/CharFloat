...
| ISS-131 | **`create_workbook` 在宿主长会话状态下失败（根因已确认：宿主状态，非代码回归）**。真机现象：`Workbooks.Add()` **返回 null 且 Add 前后 Count 都是 1**（`DisplayAlerts=true`，不是对话框阻塞），连续两次复现；**重启 WPS 后立即恢复**（`✔ 已新建工作簿 [回归检查.xlsx]`、`Add 前=2 后=3`）。判定：**长会话累积的宿主异常状态**——同一段代码今日早前建簿成功过多次。**处置**：① 建簿失败时**直接明说"宿主拒绝"**并给出「重启 WPS 重试」的可操作建议，不再报成「已新建工作簿 [null]」（那文案会被误读成代码 bug）；② 返回体新增 `hostRefused` / `workbooksBefore` / `workbooksAfter` 便于定位。**经验**：WPS 长时间高频调用后可能进入"拒绝新建文档"的状态，排查时**先重启再怀疑代码**。 | 中 | **已修（含提示优化）+ 根因已确认** | WPS 宿主 |
| ISS-132 | **`save_workbook` 触发模态框导致保存静默失败（真使用者踩到）**。真实业务使用中：`Save()` **没抛错**，但 `Saved` 一直是 `false`、**磁盘上始终没有文件**。根因由使用者确认：**保存会弹确认框**，他点了"取消"。即「调用未报错 ≠ 已保存」，与"删表/建簿路径都关了对话框"相比，**保存这条漏了**。已修：`Save()` 前后包 `DisplayAlerts=false`，并在 `finally` 恢复（不恢复会静默吞掉后续所有宿主提示）。真机验证：`✔ 已保存 saved=true`、磁盘 23161 字节 | 高 | **已修已验证** | WPS 宿主 + 我们的实现 |
| ISS-133 | **`format_cells` schema 缺 `fontName`**（真使用者踩到）。真实业务中给表头设"微软雅黑"→ 整个调用被 schema 拒绝（`arguments.fontName: 未知参数`），**其余参数（加粗/底色/字色/居中）跟着全部失效**，表现为"格式一点没生效"。宿主实现 `range.Font.Name = fontName` **本来就有**（`excel.js:1516`），只缺 schema 暴露。已补，真机验证表头读回 `true|深蓝底|白字|居中` | 高 | **已修已验证** | 契约层 |
| ISS-134 | **条件格式的比较值参数名反直觉**（真使用者踩到）。Excel/VBA 习惯写 `value1`，但本工具用 `formula1`；传 `value1` 会被 schema 拒绝。**不是缺陷但会造成实际摩擦**（真实业务里第一次就写错）。已修：在 `formula1` 的 description 里点明「比较值就放这个参数——没有 value1/value2」。真机验证：`formula1: "1"` → 条件格式读回 `=1|13551615` | 中 | **已修（描述优化）** | 契约层 |
