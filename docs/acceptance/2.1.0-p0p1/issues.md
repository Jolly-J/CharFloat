...
| ISS-113 | **MS 侧矢量形状：经工具路径组合/直线仍失败**。Lead 真机复核（新建隔离表、逐个调用）：建矩形 ✔、**分组 ✖「当前对象不允许此操作」**、**直线 ✖「当前对象不允许此操作」**、`list_shapes` ✖。**与 ms-tester 的结论存在分歧**：它依据任务窗格内置自检，称 addLine/addGroup **真的建成了**、报错来自读回（`loadShapeDetail` 对 Line/Group load `fill`）——`loadShapeDetail` 已按此修复并部署，但**工具路径下创建本身仍被宿主拒绝**。两种结论的差异可能来自「自检在窗格内、工具走 RPC」两条不同调用路径，**根因仍未定位**。**处置：以工具路径的真机读数为准**（使用者关心的就是工具能不能用）。 | 中高 | **待修（根因未定位）** | Office.js 宿主 ；**顺带补了  的  动作**（此前没有，shape 操作依赖激活状态却无法先激活；真机验证 ✔）|
...
| ISS-113 | **MS 侧矢量形状：直线/分组/形状清单失败（根因已定位，修复待真机确认）**。真机复核：建矩形 ✔、**直线 ✖ / 分组 ✖ / `list_shapes` ✖，宿主报「当前对象不允许此操作」**。**根因**：`loadShapeDetail` 一阶段写法读不到 `type` —— `type` 本身也要 `load` 才有值，所以 `String(shape.type)` 恒为 undefined → `isLine`/`isGroup` **恒为 false** → 照样对 Line/Group load `fill`/`textFrame` → `sync()` 抛错。即**读回把创建带崩了**，看起来像宿主限制。**修法**：拆成两阶段 —— 先只 `load("type")` + sync 拿到真实类型，再按类型补 load 其余属性（Line 不 load `fill`，Group 不 load `fill`/`textFrame`）；10 个调用点已改为 `loadShapeDetailTyped`。**注意**：`try/catch` 挡不住——Office.js 的 load() 是延迟的，失败发生在 sync()。**验证状态**：修复已部署到 `office-addon/public/taskpane.js`，但 **Excel 里运行中的任务窗格需重载**才能生效，重载前真机读数仍为旧行为 | 中高 | **已修待重载验证** | Office.js 加载项 |
