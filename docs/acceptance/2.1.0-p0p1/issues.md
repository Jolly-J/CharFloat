...
| ISS-126 | **残留目标锁：桥接侧修复未覆盖加载项侧的另一套锁（修复部分失效）**。真机复核：Word 文档已全部关闭后调 `wps_word_page_layout_and_watermark{action:"read"}`，**连续两次**都报「未在 WPS 中找到目标 Word 文档 [agent-word.docx]。当前已打开: 无」，**没有出现修复承诺的自动清理与可操作提示**。查证：`wps_get_locked_status` 返回 `gatewayLocks: {}` —— **桥接侧的会话锁是空的**，说明 `agent-word.docx` 这个名字来自**加载项侧的 `lockedTargets`**（`wps-addon/src/shared.js` 里独立维护的一套）。即**桥接与加载项各有一套目标锁**，ISS-126 的自动清理只判了桥接侧那套，条件①（报错匹配）成立、条件②（由会话锁注入）不成立 → 不触发。**修法方向**：桥接侧需能识别"加载项侧残留锁"，或在加载项侧也做失效校验与自动清理 | 中高 | **待修（修复未覆盖全部锁）** | 桥接 + 加载项 |
