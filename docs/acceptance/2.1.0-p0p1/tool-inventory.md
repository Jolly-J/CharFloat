...
> **📌 后续状态见 [p5/open-questions.md](p5/open-questions.md)**（2026-09-22 收口）：下面 9 条已逐条定状态 —— **6 条已被后续实测澄清**（附证据指向），**3 条仍未确认**（#4 宿主超时阈值、#6 host=microsoft 审计快照后果、#9 审计 500 条上限副作用），各自写明了下一步怎么确认。
> 下文保持**登记当时的原文**不变，以便追溯"当时为什么不知道"；已知信息以 `open-questions.md` 为准。
