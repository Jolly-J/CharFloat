...
- 运行时间：2026/9/22 20:20:41
- 目标工作簿：测试表格.xlsx（临时表 __终验__，结束时删除）
- 结果：**43/43 通过**
...
- ✔ **group_shapes 分组 + 读回成员**
  - 读数：`{"success":true,"workbookName":"测试表格.xlsx","sheetName":"__终验__","group":{"name":"v_group","type":6,"autoShapeType":-2,"l`
- ✔ **ungroup_shapes 解组**
  - 读数：`{"success":true,"workbookName":"测试表格.xlsx","sheetName":"__终验__","released":["v_title","v_text"],"releasedStillPresent":[`
- ✔ **set_shape_zorder 层级**
...
- ✔ **copy_range 复制并读回左上角**
  - 读数：`{"success":true,"workbookName":"测试表格.xlsx","source":"__终验__!A1:C5","dest":"__终验__!E1","copyType":"all","readBackFirstCel`
...
- ✔ **透视表读回（名称/数据源/字段）**
  - 读数：`{"n":"PivotTable_1790063778192","src":"=adv_高级能力实测!R129C1:R629C10"}`

### CAP-35

- ✔ **Word 页眉页脚与水印读回**
  - 读数：`文档 [测试文字文稿.docx] 共 2 节；第 1 节页眉「页眉：Office Agent Bridge 2.1 验收」页脚「页脚：MCP Sweep Word 03」；水印 无`

### CAP-21

- ✔ **update_chart 改标题 + 读回**
  - 读数：`{"name":"finProbeChart","chartType":51,"title":"终验图表","hasTitle":true,"hasLegend":true,"seriesCount":0}`
